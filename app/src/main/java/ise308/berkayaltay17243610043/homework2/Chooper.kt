...
class Chooper(brand :String,
    ...
    companion object{
        var frontTireCoefficient = 0.003
        var rearTireCoefficient = 0.005
        val TAG = "CHOOPER"
    }

    init{
        if(this.tireType == "summer"){
            Log.i(TAG,"Chooper motorcycle $brand's tire type is summer.")
            frontTireCoefficient +=0.001
            rearTireCoefficient += 0.001
        }else if(this.tireType == "winter"){
            Log.i(TAG,"Chooper motorcycle $brand's tire type is winter.")
            frontTireCoefficient -= 0.001
            rearTireCoefficient -= 0.001
        }
    }
    ...
    override fun addKm(km:Int){
        currentKm += km
        Log.i(TAG,"Chooper motorcycle '$brand' make $km km and current km is $currentKm")
    ...
    override fun calculateLifeSpan(madeKm:Int){
        ...
        if(lifespanOfFrontTire <= 0){
            Log.i(TAG,"Chooper motorcycle $brand's front tires are changed.")
            lifespanOfFrontTire = 100
            this.tireLastModifiedKm = this.currentKm
            isChanged = true
        }
        if(lifespanOfRearTire <=0){
            Log.i(TAG,"Chooper motorcycle $brand's rear tires are changed.")
            lifespanOfRearTire=100
            this.tireLastModifiedKm = this.currentKm
            isChanged = true
        }
        if(!isChanged){
            Log.i(TAG,"Chooper motorcycle $brand no need to tire change.")
        }
    }
    ...
    override fun toString(): String {
        return "Chooper Motorcycle $brand's infos;\n" +
